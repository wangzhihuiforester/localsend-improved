...
const CHUNK_SIZE: usize = 64 * 1024; // 64 KiB - 增大分块以减少传输次数，提升大文件传输效率
...
async fn receive_string_from_chunks(receive_rx: &mut mpsc::Receiver<DataChannelMessage>) -> Bytes {
    ...
    while let Some(msg) = receive_rx.recv().await {
        ...
        // 防止恶意对端发送超大控制消息导致内存耗尽
        if buffer.len() > MAX_STRING_SIZE {
            tracing::warn!("Control message exceeded {} bytes, truncating", MAX_STRING_SIZE);
            break;
        }
    }
