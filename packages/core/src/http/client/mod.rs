...
impl LsHttpClient {
    ...
    /// 向对端设备发送聊天消息。
    ///
    /// POST /api/localsend/v2/chat
    ///
    /// 注意：url 模块中没有 `build_url` 函数，实际使用 `TargetUrl` 结构体构建 URL。
    /// V2 与 V3 客户端共享底层 reqwest 客户端，聊天端点统一使用 v2 路径。
    pub async fn send_chat_message(
        ...
        // V2 和 V3 客户端共享相同的底层 reqwest 客户端
        let client = match self {
            LsHttpClient::V2(c) => &c.client,
            LsHttpClient::V3(c) => &c.client,
        };

        // 使用 TargetUrl 构建聊天端点 URL（url 模块无 build_url 函数）
        let url = TargetUrl {
