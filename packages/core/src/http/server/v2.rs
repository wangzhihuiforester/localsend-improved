...
pub enum ServerEventV2 {
    ...
    /// 通过 `POST /api/localsend/v2/chat` 接收到的聊天消息。
    ChatMessage {
        /// 发送者的 IP 地址。
        ip: PeerIp,
...
/// 处理接收到的聊天消息（`POST /api/localsend/v2/chat`）。
///
/// 解析请求体中的聊天消息，通过事件通道发送给应用层处理，并返回确认响应。
pub(crate) async fn chat(
    ...
    let payload = body.collect_to_json::<ChatRequest>().await?;

    // 将聊天消息事件发送给应用层处理
    let _ = state.v2.as_ref().map(|v2| {
        v2.event_tx.try_send(ServerEventV2::ChatMessage {
            ip: client_info.ip.clone(),
    ...
    tracing::info!("Chat message from {}: {}", payload.alias, payload.message);
...
/// 获取聊天消息列表（`GET /api/localsend/v2/chat/messages`）。
///
/// 可选端点，当前返回空列表占位，后续可扩展为持久化消息查询。
